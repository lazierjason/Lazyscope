...
/* Customized */
    removeUseless: function (doc) {
        var els = doc.getElementsByTagName('COMMENT');
		if (els && els.length > 0)
	        for(var i = els.length; i--;)
	            if(els[i] && els[i].parentNode)
					els[i].parentNode.removeChild(els[i]);
        els = doc.getElementsByTagName('NOSCRIPT');
		if (els && els.length > 0)
	        for(i = els.length; i--;)
	            if(els[i] && els[i].parentNode)
					els[i].parentNode.removeChild(els[i]);
        els = doc.getElementsByTagName('NOIFRAME');
		if (els && els.length > 0)
	        for(i = els.length; i--;)
	            if(els[i] && els[i].parentNode)
					els[i].parentNode.removeChild(els[i]);
	...
	prepareURL: function(doc) {
		if (!_LF_link || !doc) return;
		
        var els = doc.getElementsByTagName('A');
		if (els && els.length > 0) {
	        for (var i = els.length; i--;) {
	            if (!els[i].href) continue;
				if (els[i].href.match(/^\s*javascript:/i)) els[i].href='';
				else if (!els[i].href.match(/^https?:\/\//))
					els[i].href=_LF_resolveURL(_LF_link, els[i].href);
		...
		if (els && els.length > 0) {
	        for (var i = 0; i < els.length; i++) {
				...
				if (els[i].src.match(/^[\s\r\n]*javascript:/i)) els[i].src='';
				else if (!els[i].src.match(/^https?:\/\//))
					els[i].src=_LF_resolveURL(_LF_link, els[i].src);

				els[i].src = els[i].src;	// Do NOT remove this! ex) http://pddnet.com/editorial-kim-ukura-the-science-of-material-ecology-092310/
					
				if (!imgURL && els[i].src.match(/^https?:\/\//i) && !els[i].src.match(/feeds2\.feedburner\.com|feedads\.g\.doubleclick\.net|res1\.blogblog\.com|hits\.guardian\.co\.uk|ads\.guardian\.co\.uk|www\.assoc\-amazon\.com|adserver\.3digit\.de|pheedo.com\/img\.phdo|pheedo\.com\/images\/mm\/|stats\.wordpress\.com\/b\.gif|rss\.feedsportal\.com\/|da\.feedsportal\.com\/r\/|feeds\.wordpress\.com\/1\.0\/|res1\.blogblog\.com\/tracker\/|d\.techcrunch\.com\/ck\.php|d1\.openx\.org\/ck\.php|feeds\.directnews\.co\.uk\/feedtrack\/|feeds\.feedburner\.com\/\~ff|feeds\.feedburner\.com\/\~r\/|reblog\.zemanta\.com\/zemified\/|\/wp\-content\/plugins\/|api\.tweetmeme\.com\/imagebutton\.gif/))
					imgURL = els[i].src;
	        }
		}
    ...
    initCustomized: function() {
//trace('----- Readibility (START) : ', _LF_link);
		window.onunload = function(){};
		
		readability.removeUseless(document);
		readability.removeScripts(document);
		...
		var imgURL = readability.prepareURL(articleContent);
		
		var textTitle = articleTitle ? articleTitle.innerHTML.replace(/^\s+|\s+$/g, '') : '';
		var textContent = articleContent ? articleContent.innerHTML.replace(/^\s+|\s+$/g, '') : '';

////TO DEBUG///
//if (textContent.match(/code.google.com/i)) trace('@#@#@#@#:', textContent);
		LF_loadFinish(_LF_link, textTitle, textContent, imgURL);
//trace('----- Readibility (END) : ', _LF_link);
		return;
	}
};

readability.initCustomized();